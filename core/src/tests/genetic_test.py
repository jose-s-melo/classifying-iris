"""
Testes para o módulo genetic_search.py
...
GENERATIONS_RAPIDO = 2
POPULATION_RAPIDO = 3

def test_get_params_retorna_dicionario():
    """get_params() deve retornar um dicionário (dict)."""
    resultado = genetic_search.get_params(generations=GENERATIONS_RAPIDO, population_size=POPULATION_RAPIDO)
...
def test_get_params_historic_tem_tamanho_correto():
    """
    O histórico deve ter exatamente `generations * population_size` itens,
    pois cada indivíduo avaliado gera uma entrada no histórico.
    """
    resultado = genetic_search.get_params(generations=GENERATIONS_RAPIDO, population_size=POPULATION_RAPIDO)

    tamanho = len(resultado["historic"])
    esperado = GENERATIONS_RAPIDO * POPULATION_RAPIDO

    assert tamanho == esperado, (
        f"Esperava {esperado} itens no histórico, mas há {tamanho}"
    )
...
def test_get_best_mlp_retorna_mlpclassifier():
    ...
    mlp = genetic_search.get_best_mlp(generations=GENERATIONS_RAPIDO, population_size=POPULATION_RAPIDO)
...
def test_get_worst_mlp_retorna_mlpclassifier():
    ...
    mlp = genetic_search.get_worst_mlp(generations=GENERATIONS_RAPIDO, population_size=POPULATION_RAPIDO)
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("  Executando testes — MLP Genetic Search")
    print("=" * 60)
